...
Submitted: ${new Date().toLocaleString()}`;
  ...
  try {
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'BusinessHub <onboarding@resend.dev>', // You'll need to verify a domain in Resend
  ...
  // Backup: Try Web3Forms if Resend fails
  if (!emailSent) {
    try {
      const web3formsData = {
  ...
  // Always also try Formspree as backup
  try {
    ...
    // Only set emailSent if we get a clear success indicator
    if (response.ok && formspreeText.includes('Thank you')) {
      emailSent = true;
      ...
      // Don't set emailSent - let it fail
    }
  } catch (error) {
